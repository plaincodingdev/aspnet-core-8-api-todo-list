# Getting Started with ASP.NET Core 8 Web API and Docker

Welcome to this step-by-step tutorial, where you'll learn how to develop a controller-based web API using ASP.NET Core 8, and then deploy it using Docker. This guide is designed for beginners and intermediate developers interested in building and containerizing web APIs. By the end of this tutorial, you will have created a simple yet functional to-do list API.
...
- **Visual Studio Code:** This is a lightweight but powerful source code editor which runs on your desktop. It comes with support for ASP.NET C# development and debugging. Download it from Visual Studio Code official site. (https://code.visualstudio.com/)

## Creating a New Project

To create a project that is structured to use controllers—a common setup for APIs—use the following command:
...
Inside the `DTOs` folder, create two new classes, `AddToDoDto.cs` and `UpdateToDoDto.cs`, to handle the creation and updating of todo items.
...
## Creating a Service to Manage Todo Items
...
Start by defining an interface for the todo service. This interface will declare the operations that our service must implement. Create a new file named `IToDoService.cs` in a folder called `Services`:
...
Next, implement this interface in a class named `ToDoService.cs` within the same `Services` folder. This class will handle the actual logic for managing todo items:
...
## Creating a Controller for Todo Operations
